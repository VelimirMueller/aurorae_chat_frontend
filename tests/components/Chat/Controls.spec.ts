...
describe('Controls.vue', () => {
  ...
  it('emits reset event and clears input when reset button is clicked', async () => {
    const wrapper = mount(Controls, { props: { isLoading: false } })
    const input = wrapper.find('input')
    await input.setValue('to be cleared')
    const resetBtn = wrapper.findAllComponents({ name: 'AuroraeButtonType' }).find(btn => btn.text().toLowerCase().includes('clear'))
